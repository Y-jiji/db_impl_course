...
void SedaConfig::init_event_history() {
    ...
    LOG_INFO(
        "Successfully init_event_history, EventHistory:%d, MaxEventHops:%u",
...
SedaConfig::status_t SedaConfig::init_thread_pool() {
    try {
        ...
        for (size_t pos = 0; pos != name_list.size(); pos++) {
            ...
            if (thread_count < 1) {
                LOG_INFO(
                    "Thread number of  %s is %d, it is same as cpu's cores.",
...
std::string SedaConfig::get_thread_pool(std::string& stage_name) {
    ...
    if (thread_name.empty()) {
        LOG_ERROR("Failed to set %s of the %s, use the default threadpool %s",
                  thread_pool_id.c_str(), stage_name.c_str(),
                  DEFAULT_THREAD_POOL);
        return ret;
    }
...
void SedaConfig::clear_config() {
    ...
    while (s_iter != s_end) {
        if (s_iter->second != NULL) {
            ...
            ASSERT((!stg->is_connected()), "%s%s",
                   "Stage connected in clear_config ", stg->get_name());
            delete stg;
            s_iter->second = NULL;
        }
        s_iter++;
