...
int MemPoolItem::init(int item_size, bool dynamic, int pool_num,
    ...
    if (item_size <= 0 || pool_num <= 0 || item_num_per_pool <= 0) {
        LOG_ERROR("Invalid arguments, item_size:%d, pool_num:%d, "
    ...
    LOG_INFO("Extend one pool, this->size:%d, item_size:%d, "
             "item_num_per_pool:%d, this->name:%s.",
...
void MemPoolItem::cleanup() {
    ...
    for (std::list<void*>::iterator iter = pools.begin(); iter != pools.end();
         iter++) {
        void* pool = *iter;
