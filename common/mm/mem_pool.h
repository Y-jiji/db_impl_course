...
template <class T> class MemPool {
     ...
     * @return
     */
    virtual int init(bool dynamic = true, int pool_num = DEFAULT_POOL_NUM,
 ...
 */
template <class T> class MemPoolSimple : public MemPool<T> {
    public:
     ...
     * @return
     */
    int init(bool dynamic = true, int pool_num = DEFAULT_POOL_NUM,
...
int MemPoolSimple<T>::init(bool dynamic, int pool_num, int item_num_per_pool) {
    if (pools.empty() == false) {
        LOG_WARN("Memory pool has been initialized, but still begin to be "
                 "initialized, this->name:%s.",
                 this->name.c_str());
        return 0;
    }

    if (pool_num <= 0 || item_num_per_pool <= 0) {
        LOG_ERROR("Invalid arguments,  pool_num:%d, item_num_per_pool:%d, "
                  "this->name:%s.",
...
template <class T> void MemPoolSimple<T>::cleanup() {
    if (pools.empty() == true) {
        LOG_WARN(
            "Begin to do cleanup, but there is no memory pool, this->name:%s!",
            this->name.c_str());
        return;
    }
    ...
    for (typename std::list<T*>::iterator iter = pools.begin();
         iter != pools.end(); iter++) {
        T* pool = *iter;
...
template <class T> int MemPoolSimple<T>::extend() {
    if (this->dynamic == false) {
        LOG_ERROR("Disable dynamic extend memory pool, but begin to extend, "
                  "this->name:%s",
                  this->name.c_str());
        return -1;
    }
    ...
    if (pool == nullptr) {
        MUTEX_UNLOCK(&this->mutex);
        LOG_ERROR("Failed to extend memory pool, this->size:%d, "
                  "item_num_per_pool:%d, this->name:%s.",
                  this->size, item_num_per_pool, this->name.c_str());
        return -1;
    }
...
template <class T> T* MemPoolSimple<T>::alloc() {
    MUTEX_LOCK(&this->mutex);
    if (frees.empty() == true) {
        if (this->dynamic == false) {
            MUTEX_UNLOCK(&this->mutex);
...
template <class T> void MemPoolSimple<T>::free(T* buf) {
    MUTEX_LOCK(&this->mutex);
...
template <class T> T* MemPoolSimple<T>::find(match func, void* arg) {
    T* buffer = nullptr;
    MUTEX_LOCK(&this->mutex);
    for (typename std::list<T*>::iterator it = lru_used.begin();
         it != lru_used.end(); ++it) {
        T* item = *it;
...
template <class T> void MemPoolSimple<T>::mark_modified(T* buf) {
    MUTEX_LOCK(&this->mutex);
...
template <class T> std::string MemPoolSimple<T>::to_string() {
    std::stringstream ss;
