...
struct DateTime {
    ...
    inline void get_hms(int& hour, int& minute, int& second,
                        int& millis) const {
    ...
    static DateTime from_tm(const tm& tm, int millis = 0) {
        return DateTime(
            julian_date(tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday),
    ...
    static int julian_date(int year, int month, int day) {
        ...
        int y = year + 4800 - a;
        int m = month + 12 * a - 3;
        return (day + int((153 * m + 2) / 5) + y * 365 + int(y / 4) -
