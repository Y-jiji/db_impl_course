...
void LockTrace::foundDeadLock(LockID& current, LockTrace::LockID& other,
    ...
    if (itLocks == mLocks.end()) {
        ...
        LOG_ERROR("Thread %ld own mutex %p:%s:%d and try to get mutex %s:%d, "
                  ...
                  current.mThreadId, otherWaitMutex,
                  otherRecusive.mFile.c_str(), otherRecusive.mLine,
                  current.mFile.c_str(), current.mLine, other.mThreadId,
                  current.mFile.c_str(), current.mLine, otherWaitMutex,
                  otherRecusive.mFile.c_str(), otherRecusive.mLine);
    }
}
...
void LockTrace::toString(std::string& result) {
    ...
    result += "mWaitLocks:\n";
    for (std::map<long long, pthread_mutex_t*>::iterator it =
             mWaitLocks.begin();
