...
void              test_insert() {
    for (int i = 0; i < insert_num; i++) {
        ...
        if (i % TIMES == 1) {
            if (insert_num > page_size) {
                if (k++ % 100 == 0) {
                    LOG_INFO("Begin to insert the page's num %s",
                                          rid.to_string().c_str());
                }
            } else {
                LOG_INFO("Insert %d", i);
            }
            ResultCode rc = handler->insert_entry((const char*)&i, &rid);
            ASSERT_EQ(ResultCode::SUCCESS, rc);
    ...
    int end = insert_num / TIMES > page_size ? (2 * page_size)
                                                          : (2 * insert_num / TIMES);
    for (int i = start; i < end; i++) {
        ...
        if (insert_num > page_size) {
            if (k++ % 100 == 0) {
                LOG_INFO("Begin to check duplicated insert the page's num %s",
                                      rid.to_string().c_str());
            }
        } else {
        ...
        int t  = i % TIMES;
        if (t == 0 || t == 1 || t == 2) {
            ASSERT_EQ(ResultCode::RECORD_DUPLICATE_KEY, rc);
        } else {
            ASSERT_EQ(ResultCode::SUCCESS, rc);
            ASSERT_EQ(true, handler->validate_tree());
        }
...
void test_get() {
    ...
    for (int i = 0; i < insert_num; i++) {
        ...
        if (insert_num > page_size) {
            if (k++ % 100 == 0) {
                LOG_INFO("Begin to get every entry of index,  rid: %s",
                         rid.to_string().c_str());
            }
        } else {
            LOG_INFO("Begin to get every entry of index,  rid: %s",
        ...
        rids.clear();
        ResultCode rc = handler->get_entry((const char*)&i, rids);
...
void test_delete() {
    ...
    for (int i = 0; i < insert_num / 2; i++) {
        ...
        if (t == 0 || t == 1) {
            ...
            ResultCode rc = handler->delete_entry((const char*)&i, &rid);
    ...
    for (int i = 0; i < insert_num; i++) {
        ...
        if (insert_num > page_size) {
            if (k++ % 100 == 0) {
                LOG_INFO("Begin to get entry of index,  rid: %s",
                         rid.to_string().c_str());
            }
        } else {
            LOG_INFO("Begin to get entry of index,  rid: %s",
