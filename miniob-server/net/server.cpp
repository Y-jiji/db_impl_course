...
void Server::recv(int fd, short ev, void* arg) {
    ...
    while (true) {
        read_len =
            ::read(client->fd, client->buf + data_len, buf_size - data_len);
        if (read_len < 0) {
            if (errno == EAGAIN) {
                continue;
            }
...
void Server::accept(int fd, short ev, void* arg) {
    ...
    char ip_addr[24];
    if (inet_ntop(AF_INET, &addr.sin_addr, ip_addr, sizeof(ip_addr)) ==
        nullptr) {
    ...
    if (ret < 0) {
        LOG_ERROR("Failed to do event_base_set for read event of %s into "
                  "libevent, %s",
...
int Server::start_tcp_server() {
    ...
    if (server_socket_ < 0) {
        LOG_ERROR("socket(): can not create server socket: %s.",
                  strerror(errno));
        return -1;
    }
    ...
    int yes = 1;
    ret =
        setsockopt(server_socket_, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes));
    if (ret < 0) {
    ...
    if (ret < 0) {
        LOG_ERROR("Failed to set socket option non-blocking:%s. ",
                  strerror(errno));
        ::close(server_socket_);
        return -1;
    }
    ...
    if (ret < 0) {
        LOG_ERROR("listen(): can not listen server socket, %s",
                  strerror(errno));
        ::close(server_socket_);
        return -1;
    }
