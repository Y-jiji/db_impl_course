...
RC BplusTreeHandler::print_node(IndexNode *node, PageNum page_num) {
    ...
    if (node->is_leaf == false) {
        for (int i = 0; i <= node->key_num; i++) {
            ...
            BPPageHandle page_handle;
            RC rc = disk_buffer_pool_->get_this_page(file_id_, child_page_num,
                                                     &page_handle);
            if (rc != RC::SUCCESS) {
...
bool BplusTreeHandler::validate_node(IndexNode *node) {
    ...
    if (node->is_leaf && node->prev_brother != -1) {
        ...
        while (parent_page != -1) {
            ...
            RC rc = disk_buffer_pool_->get_this_page(file_id_, parent_page,
                                                     &parent_handle);
            if (rc != RC::SUCCESS) {
    ...
    for (int i = 0; i < node->key_num; i++) {
        ...
        RC rc = disk_buffer_pool_->get_this_page(file_id_, child_page,
                                                 &child_handle);
        if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::insert_entry_into_node(IndexNode *node, const char *pkey,
...
RC BplusTreeHandler::insert_intern_node(BPPageHandle &parent_page_handle,
...
RC BplusTreeHandler::split_intern_node(BPPageHandle &inter_page_handle,
                                       const char *pkey) {
...
RC BplusTreeHandler::insert_into_parent(PageNum parent_page,
                                        BPPageHandle &left_page_handle,
                                        const char *pkey,
                                        BPPageHandle &right_page_handle) {
    if (parent_page == -1) {
 ...
 * @return
 */
RC BplusTreeHandler::insert_into_new_root(BPPageHandle &left_page_handle,
                                          const char *pkey,
                                          BPPageHandle &right_page_handle) {
...
RC BplusTreeHandler::get_parent_changed_index(BPPageHandle &parent_handle,
                                              IndexNode *&parent,
                                              IndexNode *node, PageNum page_num,
                                              int &changed_index) {
    RC rc = disk_buffer_pool_->get_this_page(file_id_, node->parent,
                                             &parent_handle);
    if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::change_leaf_parent_key_insert(IndexNode *node,
                                                   int changed_indx,
                                                   PageNum page_num) {
    if (changed_indx != 0) {
        return RC::SUCCESS;
    ...
    RC rc = get_parent_changed_index(parent_handle, parent, node, page_num,
...
RC BplusTreeHandler::change_leaf_parent_key_delete(IndexNode *leaf,
                                                   int delete_indx,
...
RC BplusTreeHandler::delete_entry_from_node(IndexNode *node, const char *pkey,
...
RC BplusTreeHandler::change_insert_leaf_link(IndexNode *left, IndexNode *right,
                                             PageNum right_page) {
    ...
    if (right->next_brother != -1) {
        ...
        RC rc = disk_buffer_pool_->get_this_page(file_id_, next_right_page,
                                                 &next_right_handle);
        if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::change_delete_leaf_link(IndexNode *left, IndexNode *right,
                                             PageNum right_page) {
    if (left->is_leaf == false) {
        return RC::SUCCESS;
    }
    ...
    if (left->prev_brother != -1) {
        ...
        RC rc = disk_buffer_pool_->get_this_page(file_id_, prev_left_page,
                                                 &prev_left_handle);
        if (rc != RC::SUCCESS) {
            LOG_WARN("Failed to set link for leaf for node %s, file_id:%d",
                     file_id_, right->to_string(file_header_).c_str());
 ...
 * @return
 */
RC BplusTreeHandler::coalesce_node(BPPageHandle &parent_handle,
...
void BplusTreeHandler::change_children_parent(RID *rids, int rid_len,
    ...
    for (int i = 0; i < rid_len; i++) {
        ...
        RC rc = disk_buffer_pool_->get_this_page(file_id_, page_num,
                                                 &child_page_handle);
        if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::redistribute_nodes(BPPageHandle &parent_handle,
                                        BPPageHandle &left_handle,
...
RC BplusTreeHandler::can_merge_with_other(BPPageHandle *page_handle,
