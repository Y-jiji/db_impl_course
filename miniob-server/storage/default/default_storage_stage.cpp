...
bool DefaultStorageStage::set_properties() {
    ...
    if (ResultCode::SUCCESS != handler_->init(base_dir)) {
        LOG_ERROR("Failed to init default handler");
        return false;
    }

    ResultCode ret = handler_->create_db(sys_db);
    if (ret != ResultCode::SUCCESS && ret != ResultCode::SCHEMA_DB_EXIST) {
...
void DefaultStorageStage::handle_event(StageEvent* event) {
    ...
    case SCF_INSERT: { // insert into
        ...
        snprintf(response, sizeof(response), "%s\n",
                 rc == ResultCode::SUCCESS ? "SUCCESS" : "FAILURE");
    } break;
    case SCF_UPDATE: {
    ...
    case SCF_DELETE: {
        ...
        int            deleted_count = 0;
        rc = handler_->delete_record(current_transaction, current_db, table_name,
    ...
    if (rc == ResultCode::SUCCESS && !session->is_transaction_multi_operation_mode()) {
 ...
 */
ResultCode insert_record_from_file(Table* table, std::vector<std::string>& file_values,
    ...
    for (int i = 0; i < field_num && ResultCode::SUCCESS == rc; i++) {
        ...
        case INTS: {
            ...
            if (!deserialize_stream || !deserialize_stream.eof()) {
                ...
                rc = ResultCode::SCHEMA_FIELD_TYPE_MISMATCH;
            } else {
...
std::string DefaultStorageStage::load_data(const char* db_name,
    ...
    ResultCode                       rc              = ResultCode::SUCCESS;
    while (!fs.eof() && ResultCode::SUCCESS == rc) {
        std::getline(fs, line);
        line_num++;
