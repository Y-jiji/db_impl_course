...
void QueryCacheStage::handle_event(StageEvent *event) {
    ...
    /*
    CompletionCallback *cb = new (std::nothrow) CompletionCallback(this,
    nullptr); if (cb == nullptr) { LOG_ERROR("Failed to new callback for
    SQLStageEvent"); event->done_immediate(); return;
    }
