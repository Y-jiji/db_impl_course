...
RC create_selection_executor(Transaction* transaction, const Selects& selects, const char* db,
...
void end_transaction_if_need(Session* session, Transaction* transaction, bool all_right) {
    if (!session->is_transaction_multi_operation_mode()) {
        if (all_right) {
            transaction->commit();
        } else {
...
RC ExecuteStage::do_select(const char* db, Query* sql,
    ...
    end_transaction_if_need(session, transaction, true);
    return rc;
}
...
RC create_selection_executor(Transaction* transaction, const Selects& selects, const char* db,
    ...
    return select_node.init(transaction, table, std::move(schema),
